...
import { Globe, Smartphone, Cloud, Video, Layers, Check, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-14 space-y-12 animate-fade-up">
      ...
      <div className="text-center max-w-3xl mx-auto space-y-3">
        <Badge variant="indigo">Engineering Services</Badge>
        <h2 className="text-3xl sm:text-5xl font-extrabold text-slate-900 tracking-tight font-heading">Comprehensive Tech Services</h2>
        <p className="text-slate-600 text-xs sm:text-base">
          End-to-end digital engineering solutions tailored to accelerate startup growth.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {services.map((service) => {
          ...
          return (
            <Card key={service.id} hover padding="p-8" className="flex flex-col justify-between space-y-8">
              <div className="space-y-6">
                <div className="flex justify-between items-start">
                  <div className="p-3.5 bg-indigo-50 rounded-2xl border border-indigo-100 shadow-xs">
                ...
                <div>
                  <h3 className="text-2xl font-bold text-slate-900 mb-2 tracking-tight font-heading">{service.title}</h3>
                  <p className="text-slate-600 text-xs sm:text-sm leading-relaxed">{service.fullDesc}</p>
                </div>
              ...
              <div className="pt-6 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="text-xs text-slate-500">
                  Est. Delivery: <strong className="text-slate-900 font-mono">{service.estTimeline}</strong>
                </div>
                <Button
                  variant="primary"
                  size="sm"
